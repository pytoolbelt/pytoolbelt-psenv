...
def test_valid_environment_creation():
    env = Environment(name="dev", account="123456789012", envfile=".env", kms_key="alias/mykey", path="/params/dev")
    assert env.name == "dev"
...
def test_minimal_environment_creation():
    env = Environment(name="dev", account="123456789012", envfile=".env")
    assert env.name == "dev"
    assert env.account == "123456789012"
    assert env.envfile == ".env"
...
def test_invalid_path():
    with pytest.raises(PsenvConfigError, match="Path must start with '/'"):
        Environment(name="dev", account="123456789012", envfile=".env", path="invalid/path")


def test_invalid_kms_key():
    with pytest.raises(PsenvConfigError, match="KMS key must start with 'alias/'"):
        Environment(name="dev", account="123456789012", envfile=".env", kms_key="invalid-key")
