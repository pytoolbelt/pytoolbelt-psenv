...
def parse_args():
    ...
    init_parser.set_defaults(func=init.init_entrypoint, help="run this command once to create the ~/.psenv directory with config.")

    show_parser = sub_parsers.add_parser("show")
    show_parser.set_defaults(func=show.show_entrypoint, help="print all the configured environments in ~/.psenv/psenv.yml to the terminal")

    fetch_parser = sub_parsers.add_parser("fetch")
    fetch_parser.add_argument("-m", "--method", choices=["overwrite", "update"], default="overwrite")
    fetch_parser.set_defaults(func=fetch.fetch_entrypoint, help="pull parameters from the path in parameter store and populate an .env file.")

    push_parser = sub_parsers.add_parser("push")
    push_parser.set_defaults(func=push.push_entrypoint)
    push_parser.add_argument("-o", "--overwrite", default=False, action="store_true", help="overwrite the existing parameter store value.")
    ...
    for p in fetch_parser, push_parser, env_new_parser, env_destroy_parser, inject_parser:
        p.add_argument("-e", "--env", required=True, action=validation.ValidateEnvironmentName, help="name of the configured environment")
